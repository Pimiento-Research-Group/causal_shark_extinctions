...
dat_main <- list.files(here("data",
  ...
  mutate(coef_name = as.factor(coef_name), 
         coef_name = fct_collapse(coef_name,
                                  ...
                                  "Geographic range" = c("b_geo_dist_std", "b_range_lat_std"),
                                  Paleotemperature = c("b_temp_gat_st:temp_gat_lt2",
                                                "b_temp_gat_st:temp_gat_lt3"),
                                  Productivity = c("b_d13C_std", "b_sr_value_std"),
                                  Temperature = c("b_temp_deep_binned", 
                                                  "b_temp_gat_binned"), 
...
dat_10myr <- list.files(here("data",
  ...
  mutate(coef_name = as.factor(coef_name), 
         coef_name = fct_collapse(coef_name, 
                                  Abundance = c("b_n_genus", "b_abund"), 
                                  "Geographic range" = c("b_geo_dist_std", "b_range_lat_std"), 
                                  Paleotemperature = c("b_temp_deep_st:temp_deep_lt1", 
                                                "b_temp_deep_st:temp_deep_lt3", 
                                                "b_temp_gat_st:temp_gat_lt3"), 
                                  Productivity = c("b_d13C_std", "b_sr_value_std"), 
                                  Temperature = c("b_temp_deep_binned", 
                                                  "b_temp_gat_binned"), 
